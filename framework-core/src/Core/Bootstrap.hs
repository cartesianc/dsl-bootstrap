...
data CoreBoundaryError
  ...
  | UndeclaredCoreImport CoreSliceName String CoreSliceName String
  deriving (Eq, Show)
...
defaultCoreBoundary =
  CoreBoundary
    ...
    , CoreSlice
        ...
        , coreSliceDependsOn = []
        , coreSlicePurpose = "effect declarations, take/make/transform semantics, canonical boundaries, and send/transform contracts"
        }
    , CoreSlice
    ...
    , CoreSlice
        ...
        , coreSlicePurpose = "runtime adapter for executing the frozen core through RuntimeM, typed values, and handler dispatch"
        }
    ]
...
checkCoreBoundaryWithImportGraph :: CoreBoundary -> ImportGraph -> [CoreBoundaryError]
checkCoreBoundaryWithImportGraph boundary graph =
  unique (checkCoreBoundary boundary ++ importGraphBoundaryErrors boundary graph)
...
importGraphBoundaryErrors :: CoreBoundary -> ImportGraph -> [CoreBoundaryError]
importGraphBoundaryErrors boundary graph =
  [ UndeclaredCoreImport
      (coreSliceName sourceSlice)
      (moduleImportSourceModule currentImport)
      (coreSliceName targetSlice)
      (moduleImportTargetModule currentImport)
  | currentImport <- importGraphImports graph
  , Just sourceSlice <- [sliceForModule slices (moduleImportSourceModule currentImport)]
  , Just targetSlice <- [sliceForModule slices (moduleImportTargetModule currentImport)]
  , coreSliceName sourceSlice /= coreSliceName targetSlice
  , coreSliceName targetSlice `notElem` dependencyClosure slices [] (coreSliceName sourceSlice)
  ]
  where
    slices =
      coreBoundarySlices boundary
...
dependencyClosure :: [CoreSlice] -> [CoreSliceName] -> CoreSliceName -> [CoreSliceName]
dependencyClosure slices seen currentName
  | currentName `elem` seen =
      []
  | otherwise =
      case sliceByName slices currentName of
        Nothing ->
        ...
        Just currentSlice ->
          unique
            ( coreSliceDependsOn currentSlice
                ++ concatMap
                  (dependencyClosure slices (currentName : seen))
                  (coreSliceDependsOn currentSlice)
            )
...
sliceForModule :: [CoreSlice] -> String -> Maybe CoreSlice
sliceForModule [] _ =
  Nothing
sliceForModule (currentSlice : rest) currentModule
  | any (`sliceModuleMatches` currentModule) (coreSliceModules currentSlice) =
      Just currentSlice
  | otherwise =
      sliceForModule rest currentModule

sliceModuleMatches :: String -> String -> Bool
sliceModuleMatches pattern currentModule =
  case stripSuffixText ".*" pattern of
    Just prefix ->
      (prefix ++ ".") `isPrefixOfText` currentModule
    Nothing ->
...
renderCoreBoundaryError currentError =
  case currentError of
    ...
    UndeclaredCoreImport sourceSlice sourceModule targetSlice targetModule ->
      "undeclared core import: "
        ++ sourceModule
        ++ " in "
        ++ renderCoreSliceName sourceSlice
        ++ " imports "
        ++ targetModule
        ++ " in "
        ++ renderCoreSliceName targetSlice
...
stripSuffixText :: String -> String -> Maybe String
stripSuffixText suffix text
  | suffix `isSuffixOfText` text =
      Just (take (length text - length suffix) text)
  | otherwise =
      Nothing
...
isPrefixOfText _ [] =
  False
isPrefixOfText (expected : expectedRest) (actual : actualRest)
  | expected == actual =
      isPrefixOfText expectedRest actualRest
  | otherwise =
      False

isSuffixOfText :: String -> String -> Bool
isSuffixOfText suffix text =
  reverse suffix `isPrefixOfText` reverse text
