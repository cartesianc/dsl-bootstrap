...
data EffectSectionSeed
  ...
  | ExternalTakeSeed WorkflowFact (Maybe TypeName)
...
data ProducerStepSeed
  ...
  | TakeSeed TypeName
  | MakeSeed TypeName
  | TransformSeed TypeName TypeName TransformName
...
anaEffectSection currentSeed =
  case currentSeed of
    ...
    ExternalTakeSeed currentFact Nothing ->
      EffectTheory.externalTake currentFact
    ExternalTakeSeed currentFact (Just output) ->
      EffectTheory.externalTake currentFact output
...
anaProducerStep currentSeed =
  case currentSeed of
    ...
    TakeSeed currentInput ->
      EffectTheory.take currentInput
    MakeSeed currentOutput ->
      EffectTheory.make currentOutput
    TransformSeed currentInput currentOutput currentTransform ->
      EffectTheory.transform currentInput currentOutput currentTransform
